                                        ...
                                        <tbody>
                                            @foreach ($hasilCari as $i => $row)
                                                <tr @if ($row->stok < 10) class="row-low-stock" @endif>
                                                    ...
                                                    <td>{{ $row->bagian }}</td>
                                                    <td><span class="badge bg-{{ $row->lokasi == 'Gudang Utama' ? 'primary' : 'success' }}">{{ $row->lokasi }}</span></td>
                                                    <td>{{ $row->kategori }}</td>
                                                                ...
                                                                <td colspan="2" style="padding-left: 60px; background-color: #f8f9fa;">
                                                                                    ...
                                                                                    @if (!$hasAnyStock)
                                                                                        <tr>
                                                                                            <td colspan="7" class="text-center text-muted py-3">
                                                                                                <i class="bi bi-inbox"></i> Belum ada stok barang di Gudang Utama untuk kategori ini.
                                                                                            </td>
                                                                                        </tr>
                                                                                    @endif
                                                                                </tbody>
                                                                    ...
                                                                    @else
                                                                        <div class="alert alert-info mb-0" role="alert">
                                                                            <i class="bi bi-info-circle"></i> Belum ada barang pada kategori ini.
                                                                        </div>
                                                                    @endif
                                                                </td>
                                                            </tr>
                                                            ...
                                                            @foreach ($kategori as $k)
                                                                @php
                                                                    $barangBagian = $k->barang->filter(function ($b) use ($bg) {
                                                                        if (!$b->stokBagian) return false;
                                                                        return $b->stokBagian->where('bagian_id', $bg->id)->isNotEmpty();
                                                                    });
                                                                    if ($barangBagian->count() > 0) $adaBarang = true;
                                                                @endphp
                                                                ...
                                                                <tr id="kategori-{{ $k->id }}-bagian-{{ $bg->id }}" style="display:none;">
                                                                    <td colspan="2" style="padding-left: 60px; background-color: #f8f9fa;">
                                                                        ...
                                                                        @else
                                                                            <div class="alert alert-info mb-0" role="alert">
                                                                                <i class="bi bi-info-circle"></i> Belum ada barang pada kategori {{ $k->nama }} di bagian {{ $bg->nama }}.
                                                                            </div>
                                                                        @endif
                                                                    </td>
                                                                </tr>
                    ...
                    <div class="tab-pane fade {{ $activeTab === 'barang-kategori' ? 'show active' : '' }}" 
                            ...
                            @foreach ($kategori as $k)
                                <div class="category-section">
                                    <div class="category-header">
                                        <div>
                                            <h5 class="mb-0">
                                                <i class="bi bi-folder-fill text-primary"></i> {{ $k->nama }}
                                            </h5>
                                            <small class="text-muted">{{ $k->barang->count() }} Barang</small>
                                        </div>
                                        <div>
                        ...
                        @else
                            <div class="alert alert-warning">
                                <i class="bi bi-exclamation-triangle"></i> Belum ada kategori. Silakan tambahkan kategori terlebih dahulu.
                            </div>
                        @endif
                    </div>
                </div>
    ...
    <div class="modal fade" id="modalFilterBarang" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <form action="{{ route('admin.datakeseluruhan.index') }}" method="GET" class="modal-content">
                ...
                <div class="modal-footer">
                    <a href="{{ route('admin.datakeseluruhan.index', ['tab' => 'data-keseluruhan']) }}" class="btn btn-secondary">Reset Filter</a>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <form method="POST" id="deleteForm" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning">
                        <i class="bi bi-info-circle"></i> <strong>Perhatian:</strong> Tindakan ini akan menghapus seluruh data terkait dan tidak dapat dibatalkan.
                    </div>
                </div>
                <div class="modal-footer">
                        ...
                        searchTimeout = setTimeout(() => {
                            fetch(`{{ route('admin.api.search.barang') }}?q=${encodeURIComponent(query)}`)
                                ...
                                .then(data => {
                                    ...
                                    if (data.length === 0) {
                                        suggestionsContainer.innerHTML = '<div class="dropdown-item">Tidak ada barang ditemukan</div>';
                                        return;
                                    }

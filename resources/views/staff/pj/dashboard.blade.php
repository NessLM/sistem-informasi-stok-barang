<x-layouts.app title="Dashboard Admin" :menu="$menu">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Hitung max value untuk set skala yang proper
            const maxGudangValue = Math.max(...masukData, ...keluarData, 0);
            const suggestedMaxGudang = maxGudangValue > 0 ? undefined : 10; // Default max 10 kalau kosong
            ...
            const gudangChart = new Chart(gudangCtx, {
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            beginAtZero: true,
                            suggestedMax: suggestedMaxGudang,
                            ticks: {
                                color: '#6B7280',
                                stepSize: 1, // Paksa step 1 (0, 1, 2, 3, 4...)
            ...
            function filterGudang(filterType) {
                fetch(`${FILTER_URL}?type=gudang&filter=${filterType}`)
                    ...
                    .then(d => {
                        ...
                        // Update skala Y untuk data baru
                        const newMaxGudang = Math.max(...(d.masuk || []), ...(d.keluar || []), 0);
                        if (newMaxGudang === 0) {
                            gudangChart.options.scales.y.suggestedMax = 10;
                        } else {
                            gudangChart.options.scales.y.suggestedMax = undefined;
                        }

                        // Pastikan Y-axis tetap integer setelah update
                        gudangChart.options.scales.y.ticks.stepSize = 1;
                        gudangChart.options.scales.y.ticks.callback = function (value) {
                            return Number.isInteger(value) ? value : '';
                        };
            ...
            // ====== Filter Pengeluaran (update data + badge tahun + adjust Y-scale) ======
            function filterPengeluaran(filterType) {
                fetch(`${FILTER_URL}?type=pengeluaran&filter=${filterType}`)
                    ...
                    .then(d => {
                        ...
                        // Update skala Y berdasarkan data baru
                        const newMax = Math.max(...d.data, 0);
                        const newMin = Math.min(...d.data, 0);

                        pengeluaranChart.options.scales.y.suggestedMin = newMin > 0 ? newMin * 0.8 : 0;
                        pengeluaranChart.options.scales.y.suggestedMax = newMax > 0 ? newMax * 1.2 : 1000000;
